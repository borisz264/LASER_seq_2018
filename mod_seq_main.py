...
class mod_seq_run:
    ...
    def run_single_shapemapper(self, lib_setting, threads_per_instance):
        ...
        :return: 
        '''
        command_to_run = 'shapemapper --target %s --name %s --nproc %d --output-counted-mutations ' \
                         '--output-parsed-mutations --output-classified-mutations --out %s --temp %s --log %s --min-depth 1 --overwrite --modified --U %s' %\
    ...
    def map_one_library(self, lib_settings, threads):
        ...
        command_to_run = 'STAR --limitBAMsortRAM 20000000000 --runThreadN %d --genomeDir %s --readFilesIn %s ' \
                         '--readFilesCommand gunzip -c --outWigNorm None --outSAMtype BAM SortedByCoordinate ' \
                         '--outFilterType BySJout --outFilterMultimapNmax 1 --outWigType wiggle read1_5p ' \
                         '--outFileNamePrefix %s --outReadsUnmapped FastX --outSAMattributes All 1>>%s 2>>%s' %\
